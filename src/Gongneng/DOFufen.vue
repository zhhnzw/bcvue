<template>
  <div class="box">
    <div class="card1">
      ...
      <div id="rightbox">
        <h2>功能：<br></h2>
        &nbsp;&nbsp;&nbsp;&nbsp;对人的基因集或差异分析基因集进行DO富集分析并将分析结果进行精美图形可视化，输出图形有富集气泡图、富集条形图、富集网络图。<br>
        <br>

        <h2>输入：<br></h2>
        &nbsp;&nbsp;&nbsp;&nbsp;输入的表格文件，支持txt(制表符分隔)文本文件、csv(逗号分隔)文本文件。<br>
...
export default {
  ...
  methods: {
    file1(){
      ...
      window.open(this.Host.BASE_URL + 'public/FunctionalAnalysis/do_enrich/dodata.txt','_blank')
    },
    ...
    chartCommit(){
      ...
      if(_this.isloadfile>1){
        ...
        if(_this.fileData == null){
          ...
          _this.$axios({
            url: this.Host.BASE_URL + 'FunctionalAnalysis/do_enrich/multiUpload',
            method: 'post',
            data: _this.formData,
            headers: {
            ...
            if(res.data==='token过期'){
              ...
              _this.$axios({
                url: this.Host.BASE_URL + 'FunctionalAnalysis/do_enrich/run',
                method: 'post',
                data: _this.para,
                headers: {
