<template>
  <div class="box">
    <div class="card1">
      ...
      <div id="rightbox">
        <h2>功能：<br></h2>
        &nbsp;&nbsp;&nbsp;&nbsp;利用DO数据类型富集分析结果绘制富集差异气泡图。<br>
        <br>

        <h2>输入：<br></h2>
        &nbsp;&nbsp;&nbsp;&nbsp;格式支持txt(制表符分隔)文本文件、csv(逗号分隔)文本文件。<br>
        &nbsp;&nbsp;&nbsp;&nbsp;需要有标题，输入文件第一列必须为id号，基因差异表达倍数表：第一列为基因ID，第二列为差异表达倍数log2(FC)。<br>
        <br>
...
export default {
  ...
  methods: {
    file1(){
      ...
      window.open(this.Host.BASE_URL + 'public/FunctionalAnalysis/doenrich_buttlen/dodata.txt','_blank')
    },
    ...
    down(){
      ...
      fetch(this.pic).then(res => res.blob()).then(blob => { // 将链接地址字符内容转变成blob地址
        ...
        console.log(a.href)
        a.download = 'doenrich_buttlen.svg'  // 下载文件的名字
    ...
    chartCommit(){
      ...
      if(_this.isloadfile>1){
        ...
        if(_this.fileData == null){
          ...
          _this.$axios({
            url: this.Host.BASE_URL + 'FunctionalAnalysis/doenrich_buttlen/multiUpload',
            method: 'post',
            data: _this.formData,
            headers: {
            ...
            if(res.data==='token过期'){
              ...
              _this.$axios({
                url: this.Host.BASE_URL + 'FunctionalAnalysis/doenrich_buttlen/run',
                method: 'post',
                data: _this.para,
                headers: {
