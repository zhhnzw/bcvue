<template>
  <div class="box">
    <div class="card1">
      ...
      <div id="rightbox">
        <h2>功能：<br></h2>
        &nbsp;&nbsp;&nbsp;&nbsp;利用KEGG数据类型富集分析结果绘制富集差异气泡图。<br>
        <br>

        <h2>输入：<br></h2>
        &nbsp;&nbsp;&nbsp;&nbsp;格式支持txt(制表符分隔)文本文件、csv(逗号分隔)文本文件。<br>
        &nbsp;&nbsp;&nbsp;&nbsp;标题必须为‘ID‘ 	’Description‘	’enrich_factor‘ 	’pvalue‘ 	’gene_number‘ 标题不同将导致运行失败。<br>
        <br>
...
export default {
  ...
  methods: {
    file1(){
      ...
      window.open(this.Host.BASE_URL + 'public/FunctionalAnalysis/keggenrich_buttlen/keggbuttlen.txt','_blank')
    },
    ...
    down(){
      ...
      fetch(this.pic).then(res => res.blob()).then(blob => { // 将链接地址字符内容转变成blob地址
        ...
        console.log(a.href)
        a.download = 'keggenrich_buttlen.svg'  // 下载文件的名字
    ...
    chartCommit(){
      ...
      if(_this.isloadfile>1){
        ...
        if(_this.fileData == null){
          ...
          _this.$axios({
            url: this.Host.BASE_URL + 'FunctionalAnalysis/keggenrich_buttlen/multiUpload',
            method: 'post',
            data: _this.formData,
            headers: {
            ...
            if(res.data==='token过期'){
              ...
              _this.$axios({
                url: this.Host.BASE_URL + 'FunctionalAnalysis/keggenrich_buttlen/run',
                method: 'post',
                data: _this.para,
                headers: {
