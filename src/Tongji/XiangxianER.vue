...
export default {
  ...
  mounted() {
    ...
    this.formInline.no = 'Box_two_sig' + this.formInline.no
  },
  methods: {
    file1(){
      // window.open('https://www.baidu.com/','_blank')
      window.open(this.Host.BASE_URL + 'public/StatisticalAnalysis/saliency2/box2.txt','_blank')
    },
    ...
    down(){
      ...
      fetch(this.pic).then(res => res.blob()).then(blob => { // 将链接地址字符内容转变成blob地址
        ...
        console.log(a.href)
        a.download = 'Box_two_sig.svg'  // 下载文件的名字
    ...
    chartCommit(){
      ...
      if(_this.isloadfile>1){
        ...
        if(_this.fileData == null){
          ...
          _this.$axios({
            url: this.Host.BASE_URL + 'StatisticalAnalysis/saliency2/multiUpload',
            method: 'post',
            data: _this.formData,
            headers: {
            ...
            if(res.data==='token过期'){
              ...
              _this.$axios({
                url: this.Host.BASE_URL + 'StatisticalAnalysis/saliency2/run',
                method: 'post',
                data: _this.para,
                headers: {
