    ...
    <div class="card1">
      <div id="leftbox">
        ...
        <el-form :model="formInline" class="demo-form-inline">
          ...
          <el-form-item label="是否误差条">
            <el-select v-model="formInline.error" :placeholder="formInline.error">
      ...
      <div id="rightbox">
        ...
        &nbsp;&nbsp;&nbsp;&nbsp;3) 在箱线图上添加两样本间是否显著差异的连接线。<br>
        <br>

        <h2>输入：<br></h2>
        &nbsp;&nbsp;&nbsp;&nbsp;支持txt(制表符分隔)文本文件、csv(逗号分隔)文本文件，目前只支持到8组以内，包括8组<br>
        <br>

        <h2>参数：<br></h2>
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;1.	标题：自定义主标题，X轴和Y轴标题<br>
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;2.	误差条：决定是否显示误差条<br>
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;3.	标签：NULL表示显示数值， "p.signif"表示不显示数值，显示***<br>
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.	检测方法：1.“t.test” 2.“wilcox.test"<br>
...
export default {
  ...
  mounted() {
    ...
    this.formInline.no = 'Box_mul_sig' + this.formInline.no
  },
  methods: {
    file1(){
      // window.open('https://www.baidu.com/','_blank')
      window.open(this.Host.BASE_URL + 'public/StatisticalAnalysis/multisaliency/box.txt','_blank')
    },
    ...
    down(){
      ...
      fetch(this.pic).then(res => res.blob()).then(blob => { // 将链接地址字符内容转变成blob地址
        ...
        console.log(a.href)
        a.download = 'Box_mul_sig.svg'  // 下载文件的名字
    ...
    chartCommit(){
      ...
      if(_this.isloadfile>1){
        ...
        if(_this.formInline.error!==''){
          _this.para.error_7 = _this.formInline.error
        }
        ...
        if(_this.fileData == null){
          ...
          _this.$axios({
            url: this.Host.BASE_URL + 'StatisticalAnalysis/multisaliency/multiUpload',
            method: 'post',
            data: _this.formData,
            headers: {
            ...
            if(res.data==='token过期'){
              ...
              _this.$axios({
                url: this.Host.BASE_URL + 'StatisticalAnalysis/multisaliency/run',
                method: 'post',
                data: _this.para,
                headers: {
