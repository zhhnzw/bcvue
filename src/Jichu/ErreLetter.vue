    ...
    <div class="card1">
      <div id="leftbox">
        ...
        <el-form :model="formInline" class="demo-form-inline">
          ...
          <el-form-item label="输入分组文件">
            <el-input  v-model="formInline.file1" :placeholder="this.formInline.file1" :readonly="true"></el-input>
            <el-button type="button" id="groupfileImport" v-on:click="clickLoad2"">选择上传的数据文件</el-button>
            <input type="file" id="groupfiles" ref="refGroupFile" style="display: none" v-on:change="groupfileLoad">
      ...
      <div id="rightbox">
        ...
        &nbsp;&nbsp;&nbsp;&nbsp;注意：注意：如果一个模块中有多个信息，文件仅支持英文输入法时的";"和","作为分隔符，此时会根据分组文件同时呈现模块中的信息。<br>
        <br>
...
export default {
  ...
  data() {
    return{
      ...
      chartInstance:null,
      fileData:null, //读取的数据
      fileGroupData:null, //读取分组数据

      gro:[],

      //用户输入
      colordatafromuser: [],//分组颜色--用户自定义
      xlabelfontsize: 5,//x轴字体大小--默认为5
      ylabelfontsize: 10,//y轴字体大小--默认为10
  ...
  methods: {
    file1(){
      ...
      window.open(this.Host.BASE_URL + 'public/BaseFunction/binaryheatmap/binary_heatmap1.txt','_blank')
    },

    file2(){
      // window.open('https://www.baidu.com/','_blank')
      window.open(this.Host.BASE_URL + 'public/BaseFunction/binaryheatmap/binary_heatmap_group.txt','_blank')
    },
    ...
    chartCommit(){
      ...
      if(_this.isloadfile>1){
        ...
        _this.init_groupData(_this.fileGroupData, _this.colordatafromuser);//分组数据处理
        _this.update_chart();
    ...
    groupfileLoad() { //分组文件
      let _this = this;
      const selectedFile = this.$refs.refGroupFile.files[0];
      _this.formData.append('files',selectedFile);
      console.log(this.$refs.refGroupFile.files)
      _this.formInline.file1 = this.$refs.refGroupFile.files[0].name;
      let reader = new FileReader();
      reader.readAsText(selectedFile);
      reader.onload = function () {
        _this.fileGroupData=this.result;
      }
    },
    ...
    clickLoad2() { //分组文件
      this.$refs.refGroupFile.dispatchEvent(new MouseEvent("click"));
    },
    ...
    init_groupData(groupData, colorData){//分组数据处理
      let _this = this;
      let data_line=groupData.split("\n");//将文件按行划分并存入data_line
      ...
      for(let i=0;i<gd.length;i++){
        _this.groupdata.push({
          value: i+1, color: gdc[i], label: gn[i] //指定各数值对应颜色
        })
      }

      //修改heatmapdata中的数据
      for(let i=0;i<gd.length;i++) {
        for (let j = 0; j < _this.heatmapdata.length; j++) {
          if(_this.heatmapdata[j][2] === gd[i]){
            _this.heatmapdata[j][2] = i+1;
          }
        }
      }
    },
    ...
    update_chart(){
      ...
      const option= {
        ...
        xAxis: {
          ...
          axisLabel: {
            inside: false,
            rotate: 90,//旋转90度，竖向显示
            fontSize: _this.xlabelfontsize,//字体大小
        ...
        yAxis: {
          ...
          data: _this.ylable,
          axisLabel: {
            fontSize: _this.ylabelfontsize,
            interval: 0
          },
          position: 'right', //在图右侧显示
          splitArea: {
            show: true
          },
          axisTick: {
