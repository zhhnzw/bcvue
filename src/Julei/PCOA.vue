<template>
  <div class="box">
    <div class="card1">
      ...
      <div id="rightbox">
        <h2>功能：<br></h2>
        &nbsp;&nbsp;&nbsp;&nbsp;PCoA（principal co-ordinates analysis）主坐标分析是一种展示样本间相似性的分析方式，它的分析思路与PCA分析基本一致， 都是通过降维方式寻找复杂样本中的主要样本差异距离。与PCA不同的是，PCoA主要利用Unifrac、Bray-Curtis等样本距离信息进行计算以及降维图形展示，因此结果更集中于体现样本间的相异性距离。<br>
        <br>
...
export default {
  ...
  mounted() {
    ...
    this.formInline.no = 'PCOA' + this.formInline.no
  },
  methods: {
    file1(){
      // window.open('https://www.baidu.com/','_blank')
      window.open(this.Host.BASE_URL + 'public/ClusteringAnalysis/PCOA/pcoa_out.txt','_blank')
    },

    file2(){
      // window.open('https://www.baidu.com/','_blank')
      window.open(this.Host.BASE_URL + 'public/ClusteringAnalysis/PCOA/pcoa_group.txt','_blank')
    },
    ...
    chartCommit(){
      ...
      if(_this.isloadfile>1){
        ...
        if(_this.fileData == null){
          ...
          _this.$axios({
            url: this.Host.BASE_URL + 'ClusteringAnalysis/PCOA/multiUpload',
            method: 'post',
            data: _this.formData,
            headers: {
            ...
            if(res.data==='token过期'){
              ...
              _this.$axios({
                url: this.Host.BASE_URL + 'ClusteringAnalysis/PCOA/run',
                method: 'post',
                data: _this.para,
                headers: {
