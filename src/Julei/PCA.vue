<template>
  <div class="box">
    <div class="card1">
      <div id="leftbox">
        ...
        <el-form :model="formInline" class="demo-form-inline">
          ...
          <el-form-item label="归一化">
            <el-select v-model="formInline.scale" :placeholder="formInline.scale">
      ...
      <div id="rightbox">
        <h2>功能：<br></h2>
        &nbsp;&nbsp;&nbsp;&nbsp;将多个变量通过线性变换，筛选出数个比较重要的变量。<br>
        <br>
        <h2>上传：<br></h2>
        &nbsp;&nbsp;&nbsp;&nbsp;支持txt(制表符分隔)文本文件。如果是核酸、蛋白序列文件，必须为FASTA格式（本质是文本文件）。<br>
        文件名可由英文和数字构成，文件拓展名没有限制，可以是“.txt”、“.xlsx”、“.csv”等，例如 mydata01.txt，gene02.xlsx <br>

        <h2>输入：<br></h2>
        &nbsp;&nbsp;&nbsp;&nbsp;① 矩阵文件:文件为二维矩阵文件，第一行一般为样本，第一列为基因名。<br>
        &nbsp;&nbsp;&nbsp;&nbsp;② 列的ID:筛选用来做聚类分析的列，一般为样本名。如果不填即默认用上传矩阵文件中的全部列做聚类分析。列表中的每一行表示一个列名，且列名必须在矩阵文件中存在。<br>
        &nbsp;&nbsp;&nbsp;&nbsp;③ 行的ID:筛选用来做聚类分析的行，一般为基因名。如果不填即默认用上传矩阵文件中的全部行做聚类分析。列表中的每一行表示一个行名，且行名必须在矩阵文件中存在.<br>

        <h2>参数：<br></h2>
        &nbsp;&nbsp;&nbsp;&nbsp;归一化：我们利用z-score进行数据的归一化，即将每个基因的表达量减去这个基因在所有样本中表达量的均值，然后除以其标准差。对数据进行归一化，可消除表达量异常高的基因的影响，减少数据间的“贫富差距”。我们推荐对数据进行归一化。<br>
...
export default {
  ...
  mounted() {
    ...
    this.formInline.no = 'PCA' + this.formInline.no
  },
  methods: {
    file1(){
      // window.open('https://www.baidu.com/','_blank')
      window.open(this.Host.BASE_URL + 'public/ClusteringAnalysis/plot_PCA/PCA1.txt','_blank')
    },

    file2(){
      // window.open('https://www.baidu.com/','_blank')
      window.open(this.Host.BASE_URL + 'public/ClusteringAnalysis/plot_PCA/PCAcol.txt','_blank')
    },

    file3(){
      // window.open('https://www.baidu.com/','_blank')
      window.open(this.Host.BASE_URL + 'public/ClusteringAnalysis/plot_PCA/PCArow.txt','_blank')
    },
    ...
    chartCommit(){
      ...
      if(_this.isloadfile>1){
        ...
        //参数设置
        if(_this.formInline.file!=''&&_this.formInline.file1===''&&_this.formInline.file2===''){
          _this.para.type_1 = 0
        }else if(_this.formInline.file!=''&&_this.formInline.file1!=''&&_this.formInline.file2===''){
          _this.para.type_1 = 1
        }else if(_this.formInline.file!=''&&_this.formInline.file1===''&&_this.formInline.file2!='')(
          _this.para.type_1 = 2
        )
        ...
        if(_this.formInline.scale!=''){
          _this.para.scale_0 = _this.formInline.scale
        }
      console.log(_this.para)
        ...
        if(_this.fileData == null){
          ...
          _this.$axios({
            url: this.Host.BASE_URL + 'ClusteringAnalysis/plot_PCA/multiUpload',
            method: 'post',
            data: _this.formData,
            headers: {
            ...
            if(res.data==='token过期'){
              ...
              _this.$axios({
                url: this.Host.BASE_URL + 'ClusteringAnalysis/plot_PCA/run',
                method: 'post',
                data: _this.para,
                headers: {
