  ...
  <div class="box">
    <div class="card1">
      ...
      <div id="rightbox">
        ...
        <h2>输入：<br></h2>
        &nbsp;&nbsp;&nbsp;&nbsp;（1）两个表格可以为两个组学数据，例如基因表达量表、代谢物丰度表、微生物丰度表等。也可以是表型数据，例如升高体重等生理指标。<br>
        &nbsp;&nbsp;&nbsp;&nbsp;（2）表格需要带表头和列名，每一列代表样本，行代表各种元素，例如OTU，基因。<br>
        &nbsp;&nbsp;&nbsp;&nbsp;（3）输入文件支持txt(制表符分隔)文本文件、csv(逗号分隔)文本文件、以及Excel专用的xlsx格式。<br>
        &nbsp;&nbsp;&nbsp;&nbsp;备注：该工具计算两个表格间每行数据的相关性。
...
export default {
  ...
  mounted() {
    ...
    this.formInline.no = 'Reporticab' + this.formInline.no
  },
  methods: {
    file1(){
      // window.open('https://www.baidu.com/','_blank')
      window.open(this.Host.BASE_URL + 'public/Dynamic/plot_heatmap_cor2/heatmap1.txt','_blank')
    },

    file2(){
      // window.open('https://www.baidu.com/','_blank')
      window.open(this.Host.BASE_URL + 'public/Dynamic/plot_heatmap_cor2/heatmap2.txt','_blank')
    },
      ...
      this.$confirm('确定要提交表单吗？')
        .then(_ => {
          ...
          if(_this.paraForm.ucolor1!==''&&_this.paraForm.ucolor2!==''){
            _this.paras.ucolor_1 = 'c("' + _this.paraForm.ucolor1 + '","' + _this.paraForm.ucolor2 + '")'
          }

          if(_this.paraForm.fontsize!==''){
            _this.paras.fontsize_2 = _this.paraForm.fontsize
          }

          if(_this.paraForm.nshow!==''){
            _this.paras.nshow_3 = _this.paraForm.nshow
          }

          if(_this.paraForm.fcolor!==''){
            _this.paras.fcolor_4 = _this.paraForm.fcolor
          }

          if(_this.paraForm.fsize!==''){
            _this.paras.fsize_5 = _this.paraForm.fsize
          }

          if(_this.paraForm.gshow!==''){
            _this.paras.gshow_6 = _this.paraForm.gshow
          }

          if(_this.paraForm.rc!==''){
            _this.paras.rc_7 = _this.paraForm.rc
          }
          ...
          _this.$axios({
            url: this.Host.BASE_URL + 'Dynamic/plot_heatmap_cor2/run',
            method: 'post',
            data: _this.paras,
            headers: {
    ...
    down(){
      ...
      fetch(this.pic).then(res => res.blob()).then(blob => { // 将链接地址字符内容转变成blob地址
        ...
        console.log(a.href)
        a.download = 'reporticab.svg'  // 下载文件的名字
    ...
    chartCommit(){
      ...
      if(_this.isloadfile>1){
        ...
        if(_this.fileData == null){
          ...
          _this.$axios({
            url: this.Host.BASE_URL + 'Dynamic/plot_heatmap_cor2/multiUpload',
            method: 'post',
            data: _this.formData,
            headers: {
